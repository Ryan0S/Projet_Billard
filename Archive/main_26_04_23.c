...
void IsBall(int BallSize, int Pix_0, int Pix_max, int Lmin, int Lmax, int Total_L, int MyPM[], int Width, int Height, int* ScoreMaxR, int* ScoreMaxY, int* ScoreMaxW, int* Xr, int* Yr, int* Xw, int* Yw, int* Xy, int* Yy) {
	...
	for (int i = Pix_0 - 1; i < Pix_max; i++) {
		...
		ScoreY = 0;
			...
			if (k == Lmax - Lmin - Ballsize + 2) {
				k = 1;
				i = i + Ballsize - 1 + Total_L - Lmax + Lmin;
			}
...
int main(int argc, char *argv[]) {
	...
	const unsigned int Hmin = atoi(argv[1]);
	const unsigned int Hmax = atoi(argv[2]);
	const unsigned int Lmin = atoi(argv[3]);
	const unsigned int Lmax = atoi(argv[4]);
	...
	const int Width = Lmax - Lmin;
	const int Height = Hmax - Hmin;
	...
	//A VERIFER !!!!!!!!
	//-------
	const int Total_L = 854;
	const int Total_H = 480;
	//------

	const int Pix_0 = Hmin * Total_L + Lmin;
	const int Pix_max = (Hmax*Ballsize)*Total_L + Lmax - Ballsize;

	int ScoreMaxR=0, ScoreMaxY=0, ScoreMaxW=0;
	int Xr=0, Yr=0, Xw=0, Yw=0, Xy=0, Yy=0;
	...
	*/
	...
	IsBall(BallSize, Pix_0, Pix_max, Lmin, Lmax, Total_L, MyPM, Width, Height, &ScoreMaxR, &ScoreMaxY, &ScoreMaxW, &Xr, &Yr, &Xw, &Yw, &Xy, &Yy);
